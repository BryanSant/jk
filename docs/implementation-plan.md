...
- **v0.6 — Publishing & scripting.** ✅ Shipped. `jk publish` (GPG + Sigstore + SLSA + CycloneDX/SPDX SBOMs). `jk run script.java` (JBang-compat). `jk tool install` / `jk exec` (was `jk install` / `jkx` pre-Option-1; `jkx` is kept as an alias for `jk exec`).
