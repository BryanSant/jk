...
3. **Per step** — every step name (`compile-java`, `run-tests`, `package-jar`, `plugin-<verb>`,
   …). A plugin executes as its worker jar wrapped in an ordinary step, so this tier is also the
   per-worker/per-plugin view.
4. **Per project per step.**
...
success-only discipline as the `StepTimings` ledger and `Calibration`).
...
```json
{
  ...
  "steps": [
    { "dir": "/abs/project", "step": "compile-java",
...
`StepTimings` keys and every estimator lookup); `coord` is a display label only. Averages are
...
(`EngineServer.writeJournal`): the finished `BuildRecord`'s outcome and per-module step timings
...
Step samples with status `SKIPPED` teach nothing. Metrics fold even when `[history]` is disabled;
...
- **`jk status`** — the current project's stats (cwd) plus the machine-wide summary, with a
  per-step table (top 8 by total time; `--steps` shows all, `--global` skips the project
...
- **Web UI Status page** — KPI tiles + step table below the engine vitals, fed by
...
1. **Whole-build prior** (`BuildService.applyHistoryPrior`, applied in the ETA seed and
   ...
   pipeline on its own path and never consults the seeded ETA).
2. **Learned fixed-cost steps** (`EffortWeights.learnedFixedWeight`): steps the per-unit ledger
