...
| Per-project pin | `.jdk-version` | `.python-version` | `rust-toolchain.toml` | *(in build.gradle.kts)* | *(in pom.xml `<requireJavaVersion>`)* | Single-line file with the JetBrains feed identifier (e.g. `temurin-21`). |
...
| Git dependencies | `[sources]` `"g:a" = { git = "gh:foo/bar", tag = "v1" }` | `tool.uv.sources` git | `[dependencies.foo]` `git = "…"` | *(composite build / includeBuild)* | *(no native)* | jk hashes the canonical URL, pins SHA in the lockfile, and refuses on tag rewrites unless `jk update` is run. |
...
| Version selector default | Exact pin (Maven/Gradle muscle memory) | PEP 440 specifier set | Caret default | *(strict + dynamic; user picks)* | Exact pin | jk treats `"1.2.3"` as a hard pin. Opt into ranges with `^1.2.3` (caret), `~1.2.3` (tilde), or `>=1.2,<2`. |
