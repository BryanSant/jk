...
| Add a workspace member | `jk new <path>` / `jk add <path>` register members | `uv init` / `uv add ./lib` | `cargo new <path>` auto-registers | *(edit `settings.gradle.kts`)* | *(edit `<modules>`)* | jk, uv and cargo edit `[workspace].members` for you when run inside a workspace; gradle and maven still expect a hand-edit. |
