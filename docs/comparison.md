...
| Ephemeral tool exec | `jk tool run <coord>` (binary alias: `jkx`) | `uvx <pkg>` | — | — | — | "Resolve, cache, run, evict LRU." `jkx` is a real binary — a hardlink to `jk` with argv[0] dispatch — so it works in shebangs and CI without shell integration. |
...
| Git dependencies | `foo = { git = "gh:foo/bar", tag = "v1" }` under `[dependencies]` (coordinate discovered from the cloned repo, not declared) | `tool.uv.sources` git | `[dependencies.foo]` `git = "…"` | *(composite build / includeBuild)* | *(no native)* | jk hashes the canonical URL, pins SHA in the lockfile, and refuses on tag rewrites unless `jk update` is run. |
...
* **From uv:** the `uvx`-style ephemeral-exec UX (`jk tool run`, binary alias `jkx`), integrated Python/JDK
