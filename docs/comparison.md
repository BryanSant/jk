...
| Workspace / multi-module | `workspace { members = [...] }` in `build.jk` | `[tool.uv.workspace]` | `[workspace]` in `Cargo.toml` | `include(":a", ":b")` in `settings.gradle.kts` | `<modules>` in `pom.xml` | All five have multi-package support; jk's root-aggregated lockfile mirrors Cargo's, not Maven's per-module model. |
...
| Reproducibility check | `jk verify-reproducible` | — | *(community: cargo-bisect-rustc, manual)* | — | — | jk's verb rebuilds in a scratch dir and diffs the jar's SHA-256. |
...
| Round-trip export | `jk export pom.xml` | — | — | — | itself | jk emits a Maven-Central-grade POM from `build.jk` (no `<repositories>`, no `<build>`, BOMs flattened to `<dependencyManagement>`). Gradle export is v1.1+. |
...
| Git dependencies | `dependencies.main { "g:a" = { git = "gh:foo/bar", tag = "v1" } }` | `tool.uv.sources` git | `[dependencies.foo]` `git = "…"` | *(composite build / includeBuild)* | *(no native)* | jk hashes the canonical URL, pins SHA in the lockfile, and refuses on tag rewrites unless `jk update` is run. |
| Config file format | HOCON (`build.jk`) | TOML (`pyproject.toml`) | TOML (`Cargo.toml`) | Kotlin/Groovy DSL (`build.gradle.kts`) | XML (`pom.xml`) | HOCON gives you object substitution + comments + JSON compatibility; the file is type-checkable in IntelliJ. |
| Version selector default | Caret (`"2.18.2"` ≡ `^2.18.2`, Cargo-style) | PEP 440 specifier set | Caret default | *(strict + dynamic; user picks)* | Exact pin | jk treats `"1.2.3"` as `>=1.2.3, <2.0.0`. Imported POMs translate to `=1.2.3` to preserve Maven semantics. |
