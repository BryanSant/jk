...
arg is looked up in a static `Map`, and the array is rebuilt with the
canonical name in its place. Everything else passes through untouched.
...
1. Add an entry to `Jk.VERB_ALIASES` in
   `cli/src/main/java/dev/buildjk/cli/Jk.java`.
