...
|---|---|---|
| `jk generate` | `jk init` | Matches `mvn archetype:generate`. |
...
| `jk verify-target` | `jk verify-build` | Matches Maven's `verify` phase output naming. |
| `jk check` | `jk compile` | Pre-v1.0 name for the type-check verb. Kept for back-compat. |
| `jk install <coord>` | `jk tool install <coord>` | Pre-v1.0 surface; the CLI-tool installer moved under `jk tool` to match `uv tool install`. Two-positional rewrite. |

## How rewriting works

Aliases are rewritten *before* picocli parses argv: the first positional
...
**possibly more than one**, so `install` can expand into `tool install`
without picocli ever seeing the alias. Everything after the first
positional passes through untouched.
...
- Multi-word expansion (`install` → `tool install`) is supported. Any
  flags or positionals following the alias slot into place after the
  expanded canonical path.
- Shell tab-completion scripts generated by picocli do **not** include
  aliases. The completion menu shows canonical names only.
- `jk help <alias>` is not supported; use `jk <alias> --help` instead.

## Adding a new alias

1. Add an entry to `Jk.VERB_ALIASES` in
   `cli/src/main/java/dev/buildjk/cli/Jk.java`. The value is a
   `List<String>` — single element for a straight rename, multiple
   elements for a path expansion like `install` → `[tool, install]`.
...
Don't register aliases via picocli's `@Command(aliases = …)` — that
surfaces them in `--help`, which is what this whole scheme is designed
to avoid.
