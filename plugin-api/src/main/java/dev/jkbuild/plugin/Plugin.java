 ...
 * java.util.ServiceLoader} ({@code META-INF/services/dev.jkbuild.plugin.Plugin}).
 *
 * <p>Every plugin runs as a forked worker JVM: {@code PluginWorkerMain} on the worker's classpath
 * {@code ServiceLoader}-loads the plugin and calls {@link #run}, which emits structured results
 * through the {@link ProtocolWriter} tagged with the plugin's {@link
 * PluginManifest#protocolPrefix()}.
 */
