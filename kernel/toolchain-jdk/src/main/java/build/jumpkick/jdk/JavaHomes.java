...
public final class JavaHomes {
    ...
    public static Path resolveJavaHome(Path projectDir) {
        try {
            build.jumpkick.lock.Lockfile lock = readLockSoft(projectDir);
    ...
    private static build.jumpkick.lock.Lockfile readLockSoft(Path projectDir) {
        try {
            Path lock = projectDir.resolve("jk.lock");
            return Files.isRegularFile(lock) ? build.jumpkick.lock.LockfileReader.read(lock) : null;
        } catch (Exception e) {
            return null;
        }
    }
    ...
    private static JkBuild readBuildSoft(Path projectDir) {
        try {
            ...
            return Files.isRegularFile(toml) ? build.jumpkick.config.JkBuildParser.parse(toml) : null;
        } catch (Exception e) {
            return null;
        }
    }
