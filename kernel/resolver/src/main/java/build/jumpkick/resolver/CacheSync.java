...
import build.jumpkick.http.HostRateLimiter;
...
public final class CacheSync {
    ...
    private final build.jumpkick.repo.RepoCredentialResolver creds;
    private final boolean mirrorToM2;

    public CacheSync(Cas cas, Http http) {
        this(cas, http, new build.jumpkick.repo.RepoCredentialResolver(), false);
    }

    /** As above, with the resolving project's {@code project.m2install} value. */
    public CacheSync(Cas cas, Http http, boolean mirrorToM2) {
        this(cas, http, new build.jumpkick.repo.RepoCredentialResolver(), mirrorToM2);
    }

    /** Visible for tests — inject a credential resolver. {@code mirrorToM2} defaults to false. */
    public CacheSync(Cas cas, Http http, build.jumpkick.repo.RepoCredentialResolver creds) {
        this(cas, http, creds, false);
    }

    /** Visible for tests — inject a credential resolver and {@code mirrorToM2} explicitly. */
    public CacheSync(Cas cas, Http http, build.jumpkick.repo.RepoCredentialResolver creds, boolean mirrorToM2) {
     ...
     * completion thread for fetched / failed packages. Use this to drive a Pipeline-style progress bar
    ...
    public int syncSources(Lockfile lock, ProgressObserver observer) throws IOException, InterruptedException {
        ...
        for (PendingFetch p : pending) {
            futures.add(java.util.concurrent.CompletableFuture.supplyAsync(
                    () -> fetchSources(p, limiter), build.jumpkick.run.JkThreads.io()));
        }
    ...
    private static FetchResult fetchSources(PendingFetch p, HostRateLimiter limiter) {
        Coordinate sourcesCoord = new build.jumpkick.model.Coordinate(
    ...
    private RepoArtifactStore.IndexState repoStoreState(Lockfile.Artifact pkg) {
        String repoName = build.jumpkick.repo.RepoArtifactResolver.repoName(pkg.source());
        if (repoName == null) return RepoArtifactStore.IndexState.ABSENT;
        Coordinate coord = toCoord(pkg);
        String m2Path = build.jumpkick.repo.MavenLayout.artifactPath(coord);
        build.jumpkick.repo.RepoArtifactStore store = build.jumpkick.repo.RepoArtifactStore.forRepoName(cas.root(), repoName);
    ...
    private void healM2FromCas(Lockfile.Artifact pkg, String hex) {
        ...
        String repoName = build.jumpkick.repo.RepoArtifactResolver.repoName(pkg.source());
        if (!build.jumpkick.repo.RepoArtifactResolver.isNamedRemote(repoName)) return;
        try {
            String m2Path = build.jumpkick.repo.MavenLayout.artifactPath(toCoord(pkg));
            java.nio.file.Path target = build.jumpkick.repo.M2Dirs.localRepository().resolve(m2Path);
            var hashes = build.jumpkick.repo.M2CompatWriter.copyToM2AndHash(cas.pathFor(hex), target);
            build.jumpkick.repo.M2CompatWriter.writeMavenSidecars(target, hashes.sha1(), hashes.md5());
    ...
    private boolean materializeLocal(Lockfile.Artifact pkg, String hex) {
        ...
        build.jumpkick.repo.RepoArtifactStore local =
                build.jumpkick.repo.RepoArtifactStore.forRepoName(cas.root(), "local");
        java.util.Optional<java.nio.file.Path> jar =
                local.locate(build.jumpkick.repo.MavenLayout.artifactPath(toCoord(pkg)));
