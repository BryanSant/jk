...
public final class Versions {
    ...
    /**
     * Common pre-release qualifier tokens that Maven's {@link ComparableVersion}
     * does <em>not</em> rank below the release baseline (it treats them as
     * "unknown" qualifiers that sort <em>above</em> release): {@code pre},
     * {@code preview}, {@code dev}, {@code ea}, {@code canary}, {@code nightly}.
     * The alpha/beta/milestone/rc/cr/snapshot family is already handled by the
     * ordering check, but listing them here too is a harmless backstop. A token
     * counts only when preceded by a separator and followed by a separator,
     * digit, or end — so {@code -jre}, {@code -android}, {@code -m2e} stay stable.
     */
    ...
    /**
     * True when {@code version} is a stable release (not a pre-release such as
     * an alpha/beta/milestone/rc/snapshot/preview/dev/ea). Two complementary
     * checks, so it is robust across the qualifiers seen in the wild:
     * <ol>
     *   <li>Maven ordering — the version must sort at-or-above its own numeric
     *       core ({@code 2.4.0} for {@code 2.4.0-RC2}) per
     *       {@link ComparableVersion}. This rigorously catches the Maven-ranked
     *       pre-release family (alpha/beta/milestone/rc/cr/snapshot + aliases)
     *       while keeping release synonyms ({@code ga}/{@code final}), service
     *       packs ({@code sp}), and benign qualifiers ({@code -jre}) stable.</li>
     *   <li>A qualifier regex for pre-release markers Maven treats as unknown
     *       (and would otherwise rank above release): {@code pre}, {@code dev},
     *       {@code ea}, {@code preview}, {@code canary}, {@code nightly}.</li>
     * </ol>
     * A version with no leading numeric core ({@code "RELEASE"}, {@code "latest"})
