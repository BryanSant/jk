...
import build.jumpkick.compat.BuildTool;
import build.jumpkick.compat.ToolDistribution;
import build.jumpkick.compat.ToolInstaller;
 ...
 * {@link build.jumpkick.jdk.JavaHomes} in the slim-client Stage 5 split — the client's exec paths
 * need it engine-less).
 *
 * <p>Kotlin home order:
 *
...
public final class CompileToolchain {
     ...
     */
    public static String kotlinVersionFor(build.jumpkick.lock.Lockfile lock, JkBuild project) {
        if (lock != null && lock.kotlin() != null && !lock.kotlin().isBlank()) {
            return lock.kotlin();
        }
        if (project != null && project.project().kotlin() instanceof build.jumpkick.model.VersionSelector.Exact exact) {
