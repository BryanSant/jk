...
/**
 * The shared parent-side mechanics for driving a child-JVM plugin: fork the process, read its
 * merged stdout/stderr line by line, split structured protocol lines (those carrying the plugin's
 ...
 *       callback can use to send commands back to the plugin's stdin (e.g. a pull-queue feeding
 *       {@code RUN}/{@code DONE} in response to the plugin's {@code ready} events).
 * </ul>
 */
