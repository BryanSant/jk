...
public record CacheSnapshot(
    ...
    private static long configuredMaxBytes() {
        int gb = 20;
        try {
            gb = build.jumpkick.config.JkCacheConfig.resolve().maxSizeGb().orElse(20);
        } catch (Exception ignored) {
            // keep the default
        }
    ...
    private static long readLastPrunedMillis(Path cacheRoot) {
        Path stamp = cacheRoot.resolve(build.jumpkick.task.CachePruneScheduler.LAST_PRUNED_FILE);
