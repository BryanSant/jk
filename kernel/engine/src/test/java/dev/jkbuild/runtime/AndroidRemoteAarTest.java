...
/**
 * android-plan Phase 2 acceptance, remote half: a real androidx dependency — published as an AAR —
...
class AndroidRemoteAarTest {
    ...
    void androidx_aar_resolves_explodes_and_builds(@TempDir Path tmp) throws Exception {
        ...
        var sync = new CacheSync(new dev.jkbuild.cache.Cas(cache), new dev.jkbuild.http.Http())
                .sync(lockfile);
