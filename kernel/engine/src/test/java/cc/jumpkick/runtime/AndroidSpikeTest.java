...
class AndroidSpikeTest {
    ...
    void hello_world_apk_via_the_android_plugin(@TempDir Path tmp) throws Exception {
        ...
        var deploy = PluginCommands.run(project, cache, "deploy", List.of("--adb", fakeAdb.toAbsolutePath().toString()));
        ...
        var instrument =
                PluginCommands.run(project, cache, "instrument", List.of("--adb", instrAdb.toAbsolutePath().toString()));
