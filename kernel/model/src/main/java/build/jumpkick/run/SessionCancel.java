 ...
 * would be a cycle — this seam is how the intent "OR the session cancel into the per-pipeline poll" is
 ...
 * cancel token; {@link DefaultStepContext#cancelled()} ORs {@link #cancelled()} into its per-pipeline
 * flag, so every step that already polls {@link StepContext#cancelled()} also honors a
