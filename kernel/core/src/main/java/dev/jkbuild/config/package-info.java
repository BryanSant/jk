 ...
 * <ul>
 *   <li>{@link dev.jkbuild.config.ConfigSources} — <em>where</em> config lives and
 *       the order layers win: user-global {@code ~/.jk/config.toml} (via
 *       {@link dev.jkbuild.util.JkDirs}) then project {@code jk.toml} (or an
 *       explicit {@code --config-file}). No {@code /etc/jk} layer; jk never reads
 *       {@code ~/.config}.</li>
 *   <li>{@link dev.jkbuild.config.TomlValues} — <em>how</em> to coerce TOML values
 *       leniently (a bad value becomes "unset", never an exception).</li>
 *   <li>{@link dev.jkbuild.config.EnvValues} — <em>how</em> to coerce {@code JK_*}
 ...
 * <ul>
 *   <li>{@link dev.jkbuild.config.JkConfig} + {@link dev.jkbuild.config.JkConfigLoader}
 *       — CLI-wide scalar toggles ({@code [config]}: color, offline, quiet, …),
 *       <em>layered</em> with full project/env/CLI precedence.</li>
 *   <li>{@link dev.jkbuild.config.ForgeAuthConfig} — {@code [forge]} OAuth client
 *       IDs, <em>layered</em> (user &lt; project).</li>
 *   <li>{@link dev.jkbuild.config.JkCacheConfig} — {@code [cache]} prune policy,
 *       <em>non-project</em> (env &gt; user;
 *       {@link dev.jkbuild.config.JkCacheConfig#resolve()}).</li>
 *   <li>{@link dev.jkbuild.config.GlobalConfig} — {@code [global]} UI preferences
 ...
 *       new table). Read the value with {@link dev.jkbuild.config.TomlValues}.</li>
 *   <li>If it has a {@code JK_*} override, read that with
 *       {@link dev.jkbuild.config.EnvValues} and apply it above the file layers.</li>
