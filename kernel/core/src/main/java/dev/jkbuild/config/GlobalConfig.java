 ...
 * }</pre>
 *
 * <p>Like {@code [cache]}, {@code [global]} is <strong>not project-overridable</strong>:
 * a project {@code jk.toml}'s {@code [global]} table is ignored — these describe
 * <em>your</em> terminal/machine, not a checked-out project. Each key <em>can</em>
 * be overridden by its {@code JK_*} environment variable (env &gt; user-config &gt;
 * default), e.g. {@code JK_NERDFONT} for {@code nerdfont}, which keeps jk
 * container- and CI-friendly.
 *
 * <p>Built on the shared config SPI: values are read with {@link TomlValues} and
 * env overrides with {@link EnvValues}, so coercion matches every other config
 * view. See {@code package-info} for the full model.
 */
public final class GlobalConfig {
     ...
     * fall back to plain Unicode otherwise. Default {@code true}; opt out with
     * {@code nerdfont = false} in {@code ~/.jk/config.toml} or
     * {@code JK_NERDFONT=false} in the environment.
     *
     * <p>Resolution order: {@code JK_NERDFONT} env var → {@code [global].nerdfont}
     * in the config file → {@code true}. The env var accepts the jk-wide boolean
     * truth set ({@code 1/true/yes/on}, {@code 0/false/no/off}); see
     * {@link EnvValues}.
     */
