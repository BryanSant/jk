 ...
 * <ul>
 *   <li>{@link cc.jumpkick.config.ConfigSources} — <em>where</em> config lives and
 *       the order layers win: user-global {@code ~/.jk/config.toml} (via
 *       {@link cc.jumpkick.util.JkDirs}) then project {@code jk.toml} (or an
 *       explicit {@code --config-file}). No {@code /etc/jk} layer; jk never reads
 *       {@code ~/.config}.</li>
 *   <li>{@link cc.jumpkick.config.TomlValues} — <em>how</em> to coerce TOML values
 *       leniently (a bad value becomes "unset", never an exception).</li>
 *   <li>{@link cc.jumpkick.config.EnvValues} — <em>how</em> to coerce {@code JK_*}
 ...
 * <ul>
 *   <li>{@link cc.jumpkick.config.JkConfig} + {@link cc.jumpkick.config.JkConfigLoader}
 *       — CLI-wide scalar toggles ({@code [config]}: color, offline, quiet, …),
 *       <em>layered</em> with full project/env/CLI precedence.</li>
 *   <li>{@link cc.jumpkick.config.ForgeAuthConfig} — {@code [forge]} OAuth client
 *       IDs, <em>layered</em> (user &lt; project).</li>
 *   <li>{@link cc.jumpkick.config.JkCacheConfig} — {@code [cache]} prune policy,
 *       <em>non-project</em> (env &gt; user;
 *       {@link cc.jumpkick.config.JkCacheConfig#resolve()}).</li>
 *   <li>{@link cc.jumpkick.config.GlobalConfig} — {@code [global]} UI preferences
 ...
 *       new table). Read the value with {@link cc.jumpkick.config.TomlValues}.</li>
 *   <li>If it has a {@code JK_*} override, read that with
 *       {@link cc.jumpkick.config.EnvValues} and apply it above the file layers.</li>
