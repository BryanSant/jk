 ...
 * ~/.jk/config.toml} ({@link JkDirs#userConfigFile()}).
 *
 * <p>Cache settings are <strong>not project-overridable</strong>: a project {@code jk.toml}'s
 * {@code [cache]} table is intentionally ignored. The cache lives on your machine and is shared
 * across every project, so its size budget and prune cadence are a machine/user preference — not
 * something a checked-out project may dictate. Each key <em>can</em> be overridden by its
 * environment variable (env &gt; user-config &gt; default), which keeps jk container- and
 * CI-friendly: an image can tune the cache via {@code JK_*} without editing a file. Resolve the
 * effective config with {@link #resolve()}; the {@code (Path, env)} overload is the seam for tests.
 *
 ...
 * }</pre>
 *
 * <p>Auto-prune is <strong>on by default</strong> — the prune is opaque to the user (detached
 * subprocess, output to a sidecar log), and letting the cache grow unbounded is a worse default
 * than a background prune every 7 days. Opt out with {@code [cache]\nauto-prune = false} in {@code
 * ~/.jk/config.toml}.
 *
 * <p>Reading is intentionally lenient: malformed values fall back to the default rather than
 * failing the build. The cache layer is an optimisation, not a correctness gate.
 */
public record JkCacheConfig(boolean autoPrune, Optional<Integer> maxSizeGb, int pruneIntervalDays, int recordTtlDays) {
    ...
    /**
     * The effective cache configuration for this machine: the user-global {@code ~/.jk/config.toml}
     * (missing/malformed → {@link #DEFAULTS}), with each key overridden by its {@code JK_*}
     * environment variable when set (env &gt; user-config &gt; default). Project files are not
     * consulted.
     */
    ...
    /**
     * Load from a TOML file's {@code [cache]} table — the explicit-path seam used by {@link
     * #resolve()} and by tests. Missing file, missing table, or malformed file → {@link #DEFAULTS}.
     * Reads values through the shared {@link TomlValues} coercion; negative integers are rejected (a
