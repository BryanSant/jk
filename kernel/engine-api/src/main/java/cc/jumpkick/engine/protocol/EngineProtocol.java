 ...
 * cc.jumpkick.engine.plugin.PluginClient} already uses for jk's worker-process protocol (see {@code
 ...
 * jk releases.
 *
 * <p>Step 1: connection handshake ({@link #HELLO}/{@link #HELLO_ACK}), liveness ({@link
 ...
 * #PIPELINE_DIAGNOSTIC} exist as separate repeatable message types instead of array-valued fields on
 * {@link #PLAN_DONE}/{@link #PIPELINE_FINISH}.
 */
public final class EngineProtocol {
     ...
     * (the command failed; the message says why), {@link #ERR_PROTOCOL} (malformed request, framing
     ...
     * this: those are command output, not transport errors.
     */
    ...
    /**
     * Client → server: run a single project's test pipeline (Step 3 — {@code jk test}). A single
     * {@code Pipeline} run, not a workspace: reuses {@link #PLAN_STEP}/{@link #PLAN_DONE} (with the
     * fixed sentinel {@link #SINGLE_PIPELINE_DIR} in place of a module {@code dir}) and every {@link
     * cc.jumpkick.run.PipelineListener} event type {@link #BUILD_REQUEST} already defined — a test run
     * is exactly one pipeline, so it needs no module-plan burst or per-module tagging.
     */
    public static final String TEST_REQUEST = "test-request";

    /**
     * Client → server: run a single (non-workspace) project's build pipeline — the engine-hosted
     * counterpart of {@code BuildCommand.runForDir}. Same single-pipeline shape as {@link #TEST_REQUEST}
     * (one pipeline, {@link #SINGLE_PIPELINE_DIR}-tagged events), but {@code testOnly=false} server-side and
     * {@link #PIPELINE_FINISH} additionally carries the build outcome (see {@link
     * cc.jumpkick.runtime.BuildPipelines#BUILD_OUTCOME}) client-side rendering needs.
     */
     ...
     * #EXPLAIN_MODULE}/{@link #EXPLAIN_STEP}/{@link #EXPLAIN_EDGE}/{@link #ERROR} messages
     ...
     * and check it — the {@code jk deny} command, engine-hosted so a user-authored policy block is
    ...
    /**
     * Client → server: run a plugin-declared command (build-plugins plan row 11) for {@code dir} —
     * the CLI saw a command it doesn't own; the project's active plugin may. Synchronous inline;
     * answered with one {@link #PLUGIN_VERB_ACK} carrying {@link PluginCommandReport}
     ...
     * module is locked in declaration order, each as its own single pipeline. Wire shape per module:
     * one {@link #LOCK_MODULE}, a {@link #PLAN_STEP} burst + {@link #PLAN_DONE}, then the standard
     * {@code dir}-tagged {@link cc.jumpkick.run.PipelineListener} event vocabulary ending in a {@link
     * #PIPELINE_FINISH} that additionally carries the module's lockfile counts. {@link #LOCK_PACKAGE}
     ...
     * selects the {@code --git} splice mode, which streams no pipeline events at all — just the {@link
     ...
     * A single pipeline, so it reuses {@link #TEST_REQUEST}'s wire shape verbatim: {@link #PLAN_STEP}
     * burst ({@link #SINGLE_PIPELINE_DIR}-tagged) + {@link #PLAN_DONE}, pipeline events, then a {@link
     * #PIPELINE_FINISH} additionally carrying the fetched/up-to-date counts the summary line needs.
     * The JDK <em>install</em> half of sync's ensure-jdk step stays client-side (pre-flight): the
    ...
    //
    // The six commands that used to fork plugin workers directly from the CLI (audit, format, publish,
    // image, import, and mvn/gradle provisioning) now run engine-side: the engine assembles the same
    // pipeline the in-process test path uses (cc.jumpkick.runtime.*Pipelines), forks the worker JVM itself
    // (sized by the engine's shared HeapPlan), and streams the single-pipeline wire shape TEST_REQUEST
    // already defined — a SINGLE_PIPELINE_DIR-tagged PLAN_STEP burst + PLAN_DONE, the standard
    // PipelineListener events, then a terminal PIPELINE_FINISH variant carrying the command's structured
    ...
    /**
     * Client → server: scan {@code jk.lock} against OSV ({@code jk audit}). Single pipeline; findings
     ...
     * terminal {@code pipeline-finish} carries the counts ({@code formatTotal} of 0 = no sources found).
     */
     ...
     * engine-side. The terminal {@code pipeline-finish} carries the structured success-tail fields
     ...
     * terminal {@code pipeline-finish} carries the worker's exit code, warning count, and error text.
     */
     ...
     * jk mvn} / {@code jk gradle}). One-shot: no pipeline events, just the terminal {@link
    ...
    //
    // The in-process BuildPipelines stragglers move engine-side: jk compile, jk native, and jk
    ...
    // write into ~/.jk/bin (jk install's "make install") happens client-side after the hosted pipeline.

    /**
     * Client → server: type-check the project ({@code jk compile}) — the shared pipeline in
     * compile-only mode. Single pipeline; {@link #TEST_REQUEST}'s exact wire shape with a plain
     * {@code pipeline-finish} terminal.
     */
     ...
     * verbatim (a single project is a cascade of one): a {@link #PLAN_MODULE}/{@link #PLAN_STEP}
     * burst with per-module pipeline weights, then each module serially — {@link #MODULE_START}, pipeline
     * events, a test-count-carrying {@link #PIPELINE_FINISH}, {@link #MODULE_FINISH} with an
     ...
     * index). Single pipeline ({@link #TEST_REQUEST}'s wire shape); the terminal {@code pipeline-finish}
     * carries the test counts the exit-code logic needs. The "make install" half (launcher/binary
     * into {@code ~/.jk/bin} + {@code ~/.jk/lib}) stays client-side, after this pipeline succeeds.
     */
     ...
     * the engine's in-process git backend. Single pipeline; the terminal {@code pipeline-finish} carries
     * the checkout path + resolved sha (see {@link #pipelineFinishGitFetch}), which the client then
    ...
    // as a single pipeline, returning the resolved main class + classpath on the terminal pipeline-finish;
     ...
     * run} / {@code jk install <g:a:v>}). Single pipeline ({@link #TEST_REQUEST}'s wire shape); the
     * terminal {@code pipeline-finish} carries the resolved main class + classpath (see {@link
     * #pipelineFinishTool}), which the client feeds into its launcher write or inheritIO exec — both of
     ...
     * the caches — the Wave-3 correctness finding's fix. Single pipeline ({@link #TEST_REQUEST}'s wire
     * shape) with a {@link #pipelineFinishCache} terminal; while the engine is waiting it emits {@link
     ...
     * {@code kt} / {@code kts} / {@code jar}. Single pipeline ({@link #TEST_REQUEST}'s wire shape) with
     * a {@link #pipelineFinishScript} terminal carrying the exec ingredients; the exec itself stays
    ...
    // ---- hosted long-tail commands (Wave 4) ---------------------------------------------------------
