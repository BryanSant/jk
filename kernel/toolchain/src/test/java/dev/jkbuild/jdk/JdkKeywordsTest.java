...
class JdkKeywordsTest {
    ...
    void non_keyword_returns_empty() {
        ...
        assertThat(JdkKeywords.resolveToMajorSpec(catalog(), "25", "linux", "x86_64")).isEmpty();
    }
    ...
    void preview_majors_are_ignored() {
        JdkCatalog withPreview = new JdkCatalog(List.of(
                entry(25, "25.0.3", false, "linux", "x86_64"),
                entry(27, "27-ea+1", true, "linux", "x86_64")));
    ...
    void empty_when_no_lts_major_present() {
        ...
        assertThat(JdkKeywords.resolveToMajorSpec(noLts, "lts", "linux", "x86_64")).isEmpty();
    }
    ...
    void native_empty_when_only_community_graalvm_present() {
        JdkCatalog onlyCe = new JdkCatalog(List.of(
                graal("GraalVM", "Community Edition", "graalvm-ce-25", 25, "25.0.2")));
        assertThat(JdkKeywords.resolveToMajorSpec(onlyCe, "native", "linux", "x86_64")).isEmpty();
    }
    ...
    private static JdkCatalog.Entry graal(String vendor, String product, String sdkName,
                                          int major, String version) {
        return new JdkCatalog.Entry(
                vendor, product, sdkName, major, version,
                false, false, List.of(sdkName, version, String.valueOf(major)),
                "linux", "x86_64", "targz",
                URI.create("https://example.invalid/" + sdkName + ".tar.gz"),
                "00".repeat(32), 1234L, sdkName, "");
    }

    private static JdkCatalog catalog() {
        return new JdkCatalog(List.of(
                entry(25, "25.0.3", false, "linux", "x86_64"),
                entry(26, "26.0.1", false, "linux", "x86_64")));
    }

    private static JdkCatalog.Entry entry(int major, String version, boolean preview,
                                          String os, String arch) {
        return new JdkCatalog.Entry(
                "Eclipse", "Temurin", "temurin-" + major, major, version,
                true, preview, List.of("temurin-" + major, version, String.valueOf(major)),
                os, arch, "targz",
                URI.create("https://example.invalid/temurin-" + version + ".tar.gz"),
                "00".repeat(32), 1234L, "temurin-" + version, "");
    }
}
