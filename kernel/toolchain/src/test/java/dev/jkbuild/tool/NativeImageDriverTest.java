...
class NativeImageDriverTest {
    ...
    void executable_command_ends_with_the_main_class_and_has_no_shared_flag() {
        var req = new NativeImageDriver.Request(
                Path.of("/opt/graalvm"), List.of(Path.of("app.jar")),
                "com.example.Main", Path.of("target/widget"), List.of("--verbose"));
        ...
        assertThat(cmd).containsSequence("-o", Path.of("target/widget").toAbsolutePath().toString());
    }
    ...
    void shared_library_command_has_shared_flag_and_no_main_class() {
        var req = new NativeImageDriver.Request(
                Path.of("/opt/graalvm"), List.of(Path.of("app.jar")),
                null, Path.of("target/libwidget"), List.of(), /*shared*/ true);
        List<String> cmd = NativeImageDriver.buildCommand(BIN, req);
        ...
        assertThat(cmd).containsSequence("-o", Path.of("target/libwidget").toAbsolutePath().toString());
    }
