...
public final class PublishablePom {
    ...
    public static Pom render(BuildJk buildJk, Metadata meta) {
        ...
        appendDependencyManagement(sb, buildJk.dependencies().of(Scope.PLATFORM));
        appendDependencies(sb, buildJk);
    ...
    private static void appendDependencies(StringBuilder sb, BuildJk buildJk) {
        Scope[] order = {Scope.MAIN, Scope.RUNTIME, Scope.PROVIDED, Scope.TEST};
        boolean any = false;
        for (Scope s : order) {
            if (!buildJk.dependencies().of(s).isEmpty()) { any = true; break; }
        }
        if (!any) return;
        ...
        for (Scope s : order) {
            ...
            for (Dependency d : buildJk.dependencies().of(s)) {
                sb.append("    <dependency>\n");
                sb.append("      <groupId>").append(escape(d.group())).append("</groupId>\n");
                sb.append("      <artifactId>").append(escape(d.artifact())).append("</artifactId>\n");
                sb.append("      <version>").append(escape(versionOf(d.version()))).append("</version>\n");
