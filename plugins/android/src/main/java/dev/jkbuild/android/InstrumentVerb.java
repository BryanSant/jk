...
/**
 * The {@code instrument} verb — on-device instrumented tests (android-plan §3.6): install the app
 ...
 */
final class InstrumentVerb {

    private InstrumentVerb() {}
    ...
    private static java.util.Optional<Path> pathArg(VerbExec exec, String flag) {
    ...
    private static int adb(VerbExec exec, Path adb, boolean quiet, String... args)
            throws IOException, InterruptedException {
        return adbLines(exec, adb, quiet ? l -> {} : l -> exec.out("  " + l), args);
    }

    private static int adbLines(
            VerbExec exec, Path adb, java.util.function.Consumer<String> sink, String... args)
            throws IOException, InterruptedException {
