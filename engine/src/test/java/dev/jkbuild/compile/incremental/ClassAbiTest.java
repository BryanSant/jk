...
class ClassAbiTest {
    ...
    void body_only_change_keeps_the_abi_stable(@TempDir Path dir) throws IOException {
        byte[] v1 = JavacFixture.compileOne(dir.resolve("v1"), "a.Foo",
                "package a; public class Foo { public int f() { return 1; } }");
        byte[] v2 = JavacFixture.compileOne(dir.resolve("v2"), "a.Foo",
                "package a; public class Foo { public int f() { return 2 + 40; } }");
    ...
    void changing_a_public_constant_value_changes_the_abi(@TempDir Path dir) throws IOException {
        ...
        byte[] v1 = JavacFixture.compileOne(dir.resolve("v1"), "a.Foo",
                "package a; public class Foo { public static final int X = 1; }");
        byte[] v2 = JavacFixture.compileOne(dir.resolve("v2"), "a.Foo",
                "package a; public class Foo { public static final int X = 2; }");
        assertThat(ClassAbi.hash(v2)).isNotEqualTo(ClassAbi.hash(v1));
    }
    ...
    void changing_a_method_return_type_changes_the_abi(@TempDir Path dir) throws IOException {
        byte[] v1 = JavacFixture.compileOne(dir.resolve("v1"), "a.Foo",
                "package a; public class Foo { public int f() { return 1; } }");
        byte[] v2 = JavacFixture.compileOne(dir.resolve("v2"), "a.Foo",
                "package a; public class Foo { public long f() { return 1; } }");
        assertThat(ClassAbi.hash(v2)).isNotEqualTo(ClassAbi.hash(v1));
    }
}
