...
/**
 * Describes how a simple-task command should present itself in the console: the human command shown
 * next to the spinner ("Locking", "Syncing", …) and the final result line to print, derived from
 * the {@link PipelineResult}.
 *
 * <p>The {@code onSuccess} / {@code onFailure} mappers run after the pipeline settles, so they may
 * close over the command's pipeline/state to build a rich message (e.g. {@code "Resolved 13
 ...
 *
 * <p>{@code softFailure}, when non-null, is consulted even on a successful {@link PipelineResult}: a
 * non-null return renders the red failure chip with that exact sentence (no "Failed to &lt;command&gt;"
 * derivation), overriding the normal success rendering. For a command whose pipeline genuinely
 ...
 * success/failure decision to {@link PipelineResult#success()}.
 */
public record ConsoleSpec(
    ...
    /**
     * Default presentation — the generic {@code ✓ <Command> Successful: <msg>} finish. Set {@code chip =
     * true} (the 4-arg form) to settle through the pipeline-chip renderer instead ({@code ✓ Build ▶
     * <onSuccess>}); {@code onSuccess}/{@code onFailure} then return the tail that follows the chip's
     * command. Set both {@code chip = true} and {@code exec = true} (the 5-arg form) to settle with
