...
public final class AuthCommand implements CliCommand {
    ...
    static ForgeKind requireKind(String raw) {
        return ForgeKind.fromId(raw).orElseThrow(() ->
                new IllegalArgumentException("Unknown provider '" + raw
    ...
    static Target resolveTarget(String provider, String host, Path workingDir) {
        ...
        ForgeRemote detected = GitForgeDetector.detect(workingDir).orElseThrow(() ->
                new IllegalArgumentException("Could not detect a forge from this repo's git remote. "
