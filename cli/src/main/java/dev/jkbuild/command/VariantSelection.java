 ...
 * parks both on the ambient {@link dev.jkbuild.config.SessionContext} session. Engine request
 * writers attach the session's selection to their request lines, and in-process goal factories
 * inherit it through {@code BuildPipeline.Inputs}' session default — so a command opts in with
...
final class VariantSelection {
    ...
    static String install(Invocation in, Path projectDir) {
        ...
        dev.jkbuild.config.SessionContext.install(
                dev.jkbuild.config.SessionContext.current().withVariant(selector, clientEnv));
        return selector;
    }
