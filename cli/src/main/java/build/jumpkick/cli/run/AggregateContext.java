...
/**
 * Shared sink for a workspace build: one {@link CommandManager} (pipeline mode) that every module's
 * pipeline feeds through an {@link AggregateModuleListener}, so all modules render into a single bar +
 * step list.
 *
 * <p>Modules run sequentially. {@code completedBase} is the summed ticks of the modules that have
...
public final class AggregateContext {
    ...
    /**
     * Adjust the aggregate denominator by {@code delta} when a running module reweights a step
