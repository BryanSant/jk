...
import build.jumpkick.command.*;
import build.jumpkick.config.JkConfig;
import build.jumpkick.config.JkConfigLoader;
...
public final class Jk {
    ...
    /** Alias of {@link build.jumpkick.model.JkVersion#VERSION} for CLI-side callers. */
    public static final String VERSION = build.jumpkick.model.JkVersion.VERSION;
     ...
     * {@link build.jumpkick.cli.engine.InProcessEngine} ServiceLoader seam, because the slim client
    ...
    public static void main(String[] args) {
        ...
        if (args.length > 0 && ENGINE_SERVER_FLAG.equals(args[0])) {
            ...
            var engine = build.jumpkick.cli.engine.InProcessEngine.find().orElse(null);
            if (engine == null) {
        ...
        build.jumpkick.cli.tui.GlobalCancel.install();
    ...
    public static int execute(String... args) {
        ...
        Quietable.applyIfQuiet(build.jumpkick.config.SessionContext.current().config());
    ...
    /**
     * Argv-scan pass that folds explicit CLI flags into {@link build.jumpkick.config.SessionContext}. This is intentionally a
    ...
    private static void applyCliOverrides(String[] args) {
        ...
        build.jumpkick.config.SessionContext.installConfig(build.jumpkick.config.SessionContext.current().config().mergedWith(cli));
    }
     ...
     * build.jumpkick.config.SessionContext}. This runs before picocli parsing so the rest of the CLI
    ...
    private static void loadAndInstallConfig(String[] args) {
        ...
        try {
            ...
            build.jumpkick.config.SessionContext.installConfig(resolved);
            ...
            build.jumpkick.config.SessionContext.installConfig(JkConfig.empty());
        }
    }
