...
import build.jumpkick.tool.NativeImageDriver;
...
public final class GraalResolver {
    ...
    private Path resolveUncached(Path projectDir, String graalSpec) {
        ...
        String effective = firstNonBlank(
                build.jumpkick.config.SessionContext.current().graalSpec(), graalSpec, System.getenv("JK_GRAAL"));
        ...
        try {
            build.jumpkick.jdk.GlobalDefaultJdk gd = build.jumpkick.jdk.GlobalDefaultJdk.current();
