 ...
 * build.jumpkick.model.command.Invocation} via {@link #from(build.jumpkick.model.command.Invocation)}.
 *
 ...
 * reads {@code ~/.config} — see {@link build.jumpkick.config.ConfigSources}.
 */
public final class GlobalOptions {
     ...
     * {@link build.jumpkick.config.SessionContext}, which captures {@code -C} placed before the
    ...
    public Path workingDir() {
        Path raw = directory;
        if (raw == null) {
            raw = build.jumpkick.config.SessionContext.current().config().directory().orElse(Path.of(""));
        }
    ...
    /**
     * The global options as {@link build.jumpkick.model.command.Opt} data. The dispatcher merges these
