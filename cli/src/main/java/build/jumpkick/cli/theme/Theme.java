...
import build.jumpkick.cli.tui.Rail;
import build.jumpkick.config.JkConfig;
import org.jline.utils.AttributedString;
import org.jline.utils.AttributedStyle;

/**
 * Color and style provider used by the TUI widgets, the pipeline runner, and the help/command
 ...
 * build.jumpkick.config.SessionContext} (see {@link JkConfig.ColorChoice}):
 *
 * <ul>
...
public interface Theme {
    ...
    static boolean colorEnabled() {
        ...
        if (build.jumpkick.config.SessionContext.current().config().noAnsiOr(false)) return false;
        if ("dumb".equals(System.getenv("TERM"))) return false;
        String ci = System.getenv("CI");
        if ("true".equalsIgnoreCase(ci) || "1".equals(ci)) return false;
        var choice = build.jumpkick.config.SessionContext.current().config().colorOr(JkConfig.ColorChoice.AUTO);
