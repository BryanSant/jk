...
import build.jumpkick.tool.ToolTarget;
 ...
 * floating version selector, a miss reports both interpretations. Target kinds that later steps
...
final class ToolTargets {
    ...
    private static Resolved resolveCatalogName(ToolTarget.CatalogName c) {
        ...
        if (c.suffix() != null) {
            try {
                build.jumpkick.model.VersionSelector.parseFloating(c.suffix());
