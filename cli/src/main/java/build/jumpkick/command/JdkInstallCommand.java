...
import build.jumpkick.cli.tui.Spinner;
...
import build.jumpkick.jdk.JdkInstaller;
import build.jumpkick.jdk.JdkKeywords;
...
import build.jumpkick.jdk.JdkInstallListener;
import build.jumpkick.jdk.JdkService;
...
public final class JdkInstallCommand implements CliCommand {
    ...
    public int run(Invocation in) throws Exception {
        ...
        Step setDefault = Step.builder(StepNames.SET_DEFAULT)
                ...
                .execute(ctx -> {
                    ...
                    build.jumpkick.jdk.JdkAccessLedger.atDefaultPath().touch(installed.identifier(), "default-set");
                    CliOutput.out();
    ...
    private void offerDefaults(InstalledJdk jdk, boolean alreadyMadeDefault) {
        ...
        try (org.jline.terminal.Terminal terminal = Wizard.openTerminal()) {
            ...
            build.jumpkick.jdk.GlobalDefaultJdk defaults = build.jumpkick.jdk.GlobalDefaultJdk.current();
    ...
    private static boolean isGraalHome(Path home) {
        try {
            build.jumpkick.jdk.JdkVendor v = build.jumpkick.jdk.JdkVendor.fromRelease(home);
            return v == build.jumpkick.jdk.JdkVendor.ORACLE_GRAALVM || v == build.jumpkick.jdk.JdkVendor.GRAALVM_CE;
        } catch (RuntimeException e) {
            return false;
        }
    }
    ...
    private static String doneLine(String label, Path home, String command) {
        ...
        return build.jumpkick.cli.tui.PipelineWedge.chipLine(Glyphs.CHECK, "JDK", nerdfont, msg);
    }
     ...
     * into the animated {@link build.jumpkick.cli.tui.JdkDownloadBar} (download then installing spinner)
    ...
    private static final class InstallView implements JdkInstallListener, AutoCloseable {
        ...
        private build.jumpkick.cli.tui.JdkDownloadBar bar;
        ...
        public void onDownloadStart(String displayName, long totalBytes) {
            bar = build.jumpkick.cli.tui.JdkDownloadBar.show(CliOutput.stdout(), displayName);
        }
        ...
        public void onExtractStart(String displayName) {
            ...
            bar = build.jumpkick.cli.tui.JdkDownloadBar.showInstalling(CliOutput.stdout(), displayName);
        }
