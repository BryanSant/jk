...
import build.jumpkick.resolver.DependencyTree;
...
public final class TreeCommand implements CliCommand {
    ...
    public int run(Invocation in) throws IOException {
        ...
        if (!proj.isLocked()) {
            CliOutput.err(
                    "jk tree: no jk.lock in " + build.jumpkick.cli.PathDisplay.styledRaw(dir) + " (run `jk lock` first)");
            return Exit.CONFIG;
        }
        ...
        try {
            tagged = engineDisabledForTests()
                    ? build.jumpkick.cli.engine.InProcessEngine.require().treeRender(dir, max, flatten, stack, scopeNames)
                    : build.jumpkick.cli.engine.EngineClient.treeRender(
                            build.jumpkick.engine.EnginePaths.current(), dir, max, flatten, stack, scopeNames);
    ...
    private static DependencyTree.Styling styling(boolean nerdfont, boolean ansi) {
        ...
        UnaryOperator<String> scopeBadge = s -> build.jumpkick.cli.tui.Badge.pill(s, nerdfont);
        Theme t = Theme.active();
