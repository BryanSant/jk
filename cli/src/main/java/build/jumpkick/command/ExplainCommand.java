...
public final class ExplainCommand implements CliCommand {
    ...
    public int run(Invocation in) throws Exception {
        ...
        if (engineDisabledForTests()) {
            ...
            plan = build.jumpkick.cli.engine.InProcessEngine.require()
                    .explain(startDir,
                            build.jumpkick.cli.engine.InProcessEngine.require().parseBuild(buildFile),
            ...
            long[] etaOut = new long[1];
            plan = build.jumpkick.cli.engine.EngineClient.explain(
                    build.jumpkick.engine.EnginePaths.current(),
                    new build.jumpkick.cli.engine.EngineClient.ExplainRequest(
        ...
        int width = build.jumpkick.cli.run.PipelineConsole.isInteractiveTerminal()
                ? build.jumpkick.cli.tui.CommandManager.detectColumns()
                : Integer.MAX_VALUE;
        ...
        String header = !ansi ? " - Build Plan "
                : (nerdfont
                        ? Theme.colorize(" ≡ Build Plan ", t.planBadge())
                                + Theme.colorize(build.jumpkick.cli.tui.Glyphs.SEGMENT_END_NERD, t.bright(t.planBadgeColor()))
                        : Theme.colorize(" ≡ Build Plan ", t.planBadge()));
        ...
        if (!cachedIdx.isEmpty()) {
            ...
            String sectionBadge = ansi
                    ? build.jumpkick.cli.tui.Badge.pill("Fully Cached", nerdfont)
                    : " [Fully Cached]";
            CliOutput.out(" "
        ...
        if (!dirtyIdx.isEmpty()) {
            String rebuildBadge = ansi
                    ? build.jumpkick.cli.tui.Badge.pill("Rebuild", nerdfont)
                    : " [Rebuild]";
            CliOutput.out(" "
    ...
    private static void renderModuleRow(
        ...
        String moduleBadge = ansi
                ? build.jumpkick.cli.tui.Badge.pill(String.format("%02d", idx), nerdfont)
