...
import build.jumpkick.cli.tui.DrainView;
...
public final class EngineStopCommand implements CliCommand {
    ...
    public int run(Invocation in) {
        ...
        Optional<EngineClient.Status> before = EngineClient.status(build.jumpkick.engine.EnginePaths.activeSocket(paths));
        if (before.isEmpty()) {
        ...
        if (in.isSet("now")) {
            if (!EngineClient.forceStop(build.jumpkick.engine.EnginePaths.activeSocket(paths))) EngineClient.hardKill(before.get().pid());
            CliOutput.out(stoppedWedge(elapsed(started)));
            return Exit.SUCCESS;
        }

        // Graceful drain. The engine enters draining and reports the in-flight job count.
        int jobs = EngineClient.drain(build.jumpkick.engine.EnginePaths.activeSocket(paths));
        if (jobs <= 0) {
        ...
        try {
            while (true) {
                if (view.forceRequested()) {
                    EngineClient.forceStop(build.jumpkick.engine.EnginePaths.activeSocket(paths));
                    break;
                }
                Optional<EngineClient.Status> s = EngineClient.status(build.jumpkick.engine.EnginePaths.activeSocket(paths));
                if (s.isEmpty()) {
                    // Confirm the engine really exited (avoid a transient accept/close false positive).
                    sleep(150);
                    if (EngineClient.status(build.jumpkick.engine.EnginePaths.activeSocket(paths)).isEmpty() && !EngineClient.ping(build.jumpkick.engine.EnginePaths.activeSocket(paths))) break;
                    continue;
                }
