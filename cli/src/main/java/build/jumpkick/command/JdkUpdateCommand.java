...
import build.jumpkick.jdk.JdkCatalogClient;
import build.jumpkick.jdk.JdkGarbage;
...
public final class JdkUpdateCommand implements CliCommand {
    ...
    public int run(Invocation in) throws Exception {
        ...
        if (build.jumpkick.jdk.JdkKeywords.isKeyword(spec)) {
            var kw = build.jumpkick.jdk.JdkKeywords.bestInstalledMatch(spec, registry.managedHits(null));
            spec = kw.map(h -> {
                        Integer m = build.jumpkick.jdk.JdkKeywords.leadingMajor(h.version());
    ...
    private InstalledJdk installEntry(JdkInstaller installer, JdkCatalog.Entry entry)
        ...
        try (build.jumpkick.cli.tui.JdkDownloadBar pb = build.jumpkick.cli.tui.JdkDownloadBar.show(CliOutput.stdout(), label)) {
            installed = installer.install(entry, bytes -> pb.update(bytes, total));
            pb.finish();
        }
        build.jumpkick.jdk.JdkAccessLedger.atDefaultPath().touch(installed.identifier(), "install");
        return installed;
    }
    ...
    private boolean confirm(List<Update> updates) {
        ...
        return build.jumpkick.cli.tui.Confirm.of(Theme.colorize(Glyphs.BANG, Theme.active().warning()) + " Proceed?", true)
                .ask();
    }
