 ...
 * parks both on the ambient {@link build.jumpkick.config.SessionContext} session. Engine request
...
final class VariantSelection {
    ...
    static String install(Invocation in, Path projectDir) {
        ...
        build.jumpkick.config.SessionContext.install(
                build.jumpkick.config.SessionContext.current().withVariant(selector, clientEnv));
        return selector;
    }
