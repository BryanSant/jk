...
/**
 * Renders a {@link BuildJk} as a Maven-Central-grade {@code pom.xml} (PRD
...
public final class PomExporter {
    ...
    public static Result export(BuildJk buildJk) {
        ...
        BuildJk.Project p = buildJk.project();
        appendCoords(sb, p, buildJk.isWorkspaceRoot());
        appendProperties(sb, p);
        appendModules(sb, buildJk);
        appendRepositories(sb, buildJk.repositories());
        appendDependencyManagement(sb, buildJk.dependencies().of(Scope.PLATFORM), report);
        appendDependencies(sb, buildJk.dependencies(), report);
        appendProcessorPlugin(sb, buildJk.dependencies().of(Scope.PROCESSOR), p.jdk(), report);
        warnAboutDroppedConcerns(buildJk, report);

        sb.append("</project>\n");
        return new Result(sb.toString(), report.build());
    }

    private static void appendCoords(StringBuilder sb, BuildJk.Project p, boolean workspaceRoot) {
    ...
    private static void appendProperties(StringBuilder sb, BuildJk.Project p) {
        sb.append('\n');
    ...
    private static void appendModules(StringBuilder sb, BuildJk buildJk) {
        if (!buildJk.isWorkspaceRoot()) return;
        sb.append('\n');
        sb.append("  <modules>\n");
        for (String member : buildJk.workspace().members()) {
    ...
    private static void appendDependencies(StringBuilder sb, BuildJk.Dependencies deps,
                                            ImportReport.Builder report) {
    ...
    private static void warnAboutDroppedConcerns(BuildJk buildJk, ImportReport.Builder report) {
        if (!buildJk.features().isEmpty()) {
            report.warning("`features` block dropped — features are jk-only consumer-side"
                    + " selectors; not a POM concept (PRD §21.3).");
        }
        if (buildJk.profiles() != null && !buildJk.profiles().byName().isEmpty()) {
